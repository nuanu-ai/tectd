...
For every work candidate, verify that its accepted outcome and acceptance evidence identify a supported consumer path from an entry or delivery boundary to the promised result for the intended consumer. A user action or API, a public library contract, a job or event path, an operational procedure, or an explicitly requested artifact can establish that path. Prerequisites may remain, but later work must not be needed merely to connect this contribution to the consumer. Direct tests of a new internal implementation alone do not establish the path.

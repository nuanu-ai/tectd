...
Those downstream publishing pipelines are outside the current seven-Slice catalogue and are not natively callable here. The honest current outcome is therefore proposal, rejection, block or handoff unless external evidence already exists. A handoff is executable by an available parent/agent action only when it names the real owner and target, provides source/proof/authority/conditions, states the expected evidence to return, and does not invoke a nonexistent V1 route. No new durable-domain, runbook-library or skill-authoring implementation is part of this delivery.

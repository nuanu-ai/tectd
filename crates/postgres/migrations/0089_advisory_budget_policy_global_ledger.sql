-- All JEV routes spend one workspace/policy interval, not one opportunity.
-- Both start paths and these triggers serialize on the immutable policy row.
CREATE FUNCTION advisory_budget_policy_usage_totals(
...
CREATE FUNCTION advisory_budget_global_reservation_guard() RETURNS trigger
...
BEGIN
    ...
    RETURN NEW;
END; $guard$;
CREATE TRIGGER zz_advisory_budget_global_reservation_guard
    BEFORE INSERT ON advisory_budget_reservations FOR EACH ROW
...
CREATE FUNCTION advisory_budget_global_consumption_guard() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $guard$
DECLARE p public.advisory_budget_policies%ROWTYPE;
DECLARE u record;
...
BEGIN
    ...
    RETURN NEW;
END; $guard$;
CREATE TRIGGER zz_advisory_budget_global_consumption_guard
...
CREATE TRIGGER zz_anti_bloat_budget_global_consumption_guard
    ...
    EXECUTE FUNCTION advisory_budget_global_consumption_guard();
REVOKE ALL PRIVILEGES ON FUNCTION advisory_budget_policy_usage_totals(uuid,uuid,uuid,bigint,text) FROM PUBLIC;
REVOKE ALL PRIVILEGES ON FUNCTION advisory_budget_global_reservation_guard() FROM PUBLIC;
REVOKE ALL PRIVILEGES ON FUNCTION advisory_budget_global_consumption_guard() FROM PUBLIC;

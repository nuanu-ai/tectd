...
pub(crate) async fn publish(
    ...
    match change.operation {
        KnowledgeOperation::Create | KnowledgeOperation::Revise => {
            ...
            sqlx::query("INSERT INTO knowledge_unit_heads(tenant_id,workspace_id,unit_id,accepted_revision,active,proposal_fingerprint,last_event_id) VALUES($1,$2,$3,$4,true,$5,$6) ON CONFLICT(tenant_id,workspace_id,unit_id) DO UPDATE SET accepted_revision=EXCLUDED.accepted_revision,active=true,proposal_fingerprint=EXCLUDED.proposal_fingerprint,last_event_id=EXCLUDED.last_event_id,updated_at=pg_catalog.clock_timestamp()")
        ...
        KnowledgeOperation::Retract => {
            sqlx::query("UPDATE knowledge_unit_heads SET active=false,last_event_id=$4,updated_at=pg_catalog.clock_timestamp() WHERE tenant_id=$1 AND workspace_id=$2 AND unit_id=$3")

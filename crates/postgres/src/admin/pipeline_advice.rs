...
pub(super) async fn validate_pipeline_open_effect_schema(
    transaction: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    runtime_role: &str,
) -> Result<()> {
    let ready: bool = sqlx::query_scalar(
        "SELECT c.relrowsecurity AND c.relforcerowsecurity \
            ...
            AND pg_catalog.has_table_privilege($1,'public.pipeline_open_effect_attestations','SELECT') \
            AND pg_catalog.has_table_privilege($1,'public.pipeline_open_effect_attestations','INSERT') \
            AND NOT pg_catalog.has_table_privilege($1,'public.pipeline_open_effect_attestations','UPDATE') \
            AND NOT pg_catalog.has_table_privilege($1,'public.pipeline_open_effect_attestations','DELETE') \
            AND (SELECT pg_catalog.count(*)=2 FROM pg_catalog.pg_trigger t \
                 WHERE t.tgrelid=c.oid AND NOT t.tgisinternal \
                   AND t.tgname IN ('pipeline_open_effect_active_verifier','pipeline_open_effect_immutable')) \
         FROM pg_catalog.pg_class c JOIN pg_catalog.pg_roles r ON r.rolname=$1 \
         WHERE c.oid='public.pipeline_open_effect_attestations'::regclass",
    )
    .bind(runtime_role)

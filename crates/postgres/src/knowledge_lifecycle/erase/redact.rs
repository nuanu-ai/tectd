...
pub(super) async fn registered(
    ...
    for (id, relation, row, revision, operation, request) in copies {
        ...
        let result=match relation.as_str(){
            ...
            "pipeline_knowledge_manifests"=>sqlx::query("UPDATE pipeline_knowledge_manifests SET digest=NULL,semantic_digest=NULL,selected=NULL,unresolved_needs=NULL,definition_version=NULL,definition_digest=NULL,method_requirements=NULL,selected_resources=NULL,resource_unresolved_needs=NULL,freshness_warnings=NULL,resource_semantic_digest=NULL,resource_inquiry=NULL,resource_projection_policy=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3").bind(tenant).bind(workspace).bind(row).execute(&mut **tx).await,
            "slice_pipeline_runs"=>sqlx::query("UPDATE slice_pipeline_runs SET origin_payload=NULL,origin_result=NULL,qualification_reason=NULL,inquiry=NULL,source_checkpoint_digest=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3").bind(tenant).bind(workspace).bind(row).execute(&mut **tx).await,
            ...
            "slice_pipeline_inputs"=>sqlx::query("UPDATE slice_pipeline_inputs SET input='[erased]',input_digest=NULL,request_payload=NULL,result_payload=NULL,checkpoint_digest=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3").bind(tenant).bind(workspace).bind(row).execute(&mut **tx).await,
            "pipeline_research_checkpoints"=>{
                sqlx::query("UPDATE native_slices SET source_checkpoint_id=NULL,source_checkpoint_digest=NULL WHERE tenant_id=$1 AND workspace_id=$2 AND source_checkpoint_id=$3")
                    .bind(tenant).bind(workspace).bind(row).execute(&mut **tx).await.map_err(storage_error)?;
                sqlx::query("UPDATE pipeline_research_checkpoints SET digest=NULL,producer_output_digest=NULL,basis=NULL,question=NULL,answer_criteria=NULL,inquiry=NULL,reason=NULL,consumer_terminal_output_digest=NULL,resolution_reason=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3").bind(tenant).bind(workspace).bind(row).execute(&mut **tx).await
            },
            "pipeline_checkpoint_receipts"=>sqlx::query("UPDATE pipeline_checkpoint_receipts SET request_payload=NULL,result_payload=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND checkpoint_id=$3 AND request_id=$4").bind(tenant).bind(workspace).bind(row).bind(request.ok_or(Error::InternalInvariant)?).execute(&mut **tx).await,
            ...
            "scope_candidate_drafts"=>sqlx::query("UPDATE scope_candidate_drafts SET payload=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND candidate_set_id=$3 AND set_revision=$4").bind(tenant).bind(workspace).bind(row).bind(revision).execute(&mut **tx).await,
            "scope_candidate_reviews"=>sqlx::query("UPDATE scope_candidate_reviews SET payload=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND candidate_set_id=$3 AND set_revision=$4").bind(tenant).bind(workspace).bind(row).bind(revision).execute(&mut **tx).await,
            ...
            "scope_candidate_receipts"=>sqlx::query("UPDATE scope_candidate_receipts SET request_payload=NULL,result_payload=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND candidate_set_id=$3 AND operation=$4 AND request_id=$5").bind(tenant).bind(workspace).bind(row).bind(operation.ok_or(Error::InternalInvariant)?).bind(request.ok_or(Error::InternalInvariant)?).execute(&mut **tx).await,
            ...
            "knowledge_maintenance_signals"=>sqlx::query("UPDATE knowledge_maintenance_signals SET basis=NULL,basis_digest=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3").bind(tenant).bind(workspace).bind(row).execute(&mut **tx).await,
            "knowledge_maintenance_tasks"=>sqlx::query("UPDATE knowledge_maintenance_tasks SET state='obsolete',lease_token=NULL,lease_expires_at=NULL,next_retry_at=NULL,failure_code=NULL,current_review=NULL,affected_consumers=NULL,terminal_evidence=NULL,payload_erased=true,revision=revision+1,updated_at=pg_catalog.clock_timestamp() WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3").bind(tenant).bind(workspace).bind(row).execute(&mut **tx).await,
            "knowledge_maintenance_command_receipts"=>sqlx::query("UPDATE knowledge_maintenance_command_receipts SET request_payload=NULL,result_payload=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3").bind(tenant).bind(workspace).bind(row).execute(&mut **tx).await,
            "knowledge_maintenance_consumers"=>sqlx::query("DELETE FROM knowledge_maintenance_consumers WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3").bind(tenant).bind(workspace).bind(row).execute(&mut **tx).await,
            "planning_knowledge_manifests"=>sqlx::query("UPDATE planning_knowledge_manifests SET task_context_digest=NULL,task_context=NULL,needs=NULL,selected=NULL,unresolved_needs=NULL,digest=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3").bind(tenant).bind(workspace).bind(row).execute(&mut **tx).await,
            "planning_knowledge_consumptions"=>sqlx::query("UPDATE planning_knowledge_consumptions SET redacted=true WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3").bind(tenant).bind(workspace).bind(row).execute(&mut **tx).await,
            "program_knowledge_refresh_receipts"=>sqlx::query("UPDATE program_knowledge_refresh_receipts SET request_payload=NULL,manifest_id=NULL,result_payload=NULL,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND program_id=$3 AND request_id=$4").bind(tenant).bind(workspace).bind(row).bind(request.ok_or(Error::InternalInvariant)?).execute(&mut **tx).await,
            "programs"=>sqlx::query("UPDATE programs SET status='draft',name=NULL,intent=NULL,basis=NULL,boundaries=NULL,constraints=NULL,success=NULL,working_notes=NULL,pending_question='needs_context: knowledge payload erased',current_step='waiting_input',payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3 AND revision=$4").bind(tenant).bind(workspace).bind(row).bind(revision).execute(&mut **tx).await,
            "native_scopes"=>sqlx::query("UPDATE native_scopes SET title='[knowledge_payload_erased]',outcome='needs_context: knowledge payload erased',includes='[]'::jsonb,excludes='[]'::jsonb,payload_erased=true WHERE tenant_id=$1 AND workspace_id=$2 AND id=$3 AND revision=$4").bind(tenant).bind(workspace).bind(row).bind(revision).execute(&mut **tx).await,

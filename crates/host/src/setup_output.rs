...
pub(crate) fn reload(id: Uuid) -> Value {
    action(
        "get_setup",
...
fn save_action(setup: &Setup, cursor: i64) -> Value {
    let mut result = action(
        "save_setup",
        json!({"setup_id":setup.id,
        "revision":setup.revision,"input_cursor":cursor,"ready":false}),
    );
    result["input"] = json!({"fields":["content","working_notes","pending_question","ready"],
        "format":"content is the whole proposed AGENTS.md. Optional string-or-null patches: omission preserves, null clears. ready is a required boolean; true requires coherent content, all inputs incorporated and no pending question."});
    result
}

fn actions(setup: &Setup, delivered: Option<i64>, next: Option<i64>) -> Vec<Value> {
    let mut calls = vec![action("read_skill", json!({"name":"tectd-setup"}))];
...
fn instruction(step: SetupStep) -> &'static str {
    match step {
        ...
        SetupStep::ReadyToApply => {
            "Before calling apply_setup, send the entire saved setup.content verbatim to the user in a commentary code block. The tool result alone and a display after application do not satisfy this step. If this exact content was already shown, do not duplicate it. All inputs are incorporated and no question is pending; then apply the exact ready revision under existing setup authorization without an extra approval ceremony. On an uncertain result, retry the same setup_id and ready revision. Report creation only after current byte verification succeeds."
        }
        SetupStep::Complete => {
...
fn base(setup: &Setup, file: Value, delivered: Option<i64>, next: Option<i64>) -> Value {
    with_actions(
...
pub(crate) fn saved(setup: Setup) -> Value {
    base(
        &setup,
...
fn page_value(page: &SetupPage) -> Value {
...
pub(crate) fn page(mut page: SetupPage, capacity: usize) -> Result<Value> {
    ...
    let count = crate::program_output::paging::fitting_prefix(
        ...
        |prefix, more| {
            ...
            (actions(&page.setup, Some(last), next), json!(next))
        },
    )?;
...
pub(crate) fn applied(result: AppliedSetup) -> Value {
    base(
        &result.setup,

        ...
        let (name, arguments) = match routed {
            ...
            Err(_) => {
                let candidate_route = match (
                    ...
                    ("query", Some("engineering.matrix.planning_effect.get")) => {
                        Some("get_matrix_planning_effect")
                    }
                    ("command", Some("engineering.matrix.planning_effect.verify")) => {
                        Some("verify_matrix_planning_effect")
                    }
                    ("query", Some("pipeline.open_effect.get")) => Some("get_pipeline_open_effect"),
                    ("command", Some("pipeline.open_effect.verify")) => {
                        Some("verify_pipeline_open_effect")
                    }
                    ("query", Some("pipeline.phase_effect.get")) => {
                        Some("get_pipeline_phase_effect")
                    }
                    ("command", Some("pipeline.phase_effect.verify")) => {
                        Some("verify_pipeline_phase_effect")
                    }
